...
func calculate(lOperand string, rOperand string, operator string) (int64, error) {
	var leftConverted, erLeft = strconv.ParseInt(lOperand, 10, 64)
	if erLeft != nil {
		return 0, errors.New("[Error]: Can't parse the left value")
	}
	var rightConverted, erRight = strconv.ParseInt(rOperand, 10, 64)
	if erRight != nil {
		return 0, errors.New("[Error]: Can't parse the right value")
	}
	switch operator {
	case "*":
		return multiply{}.Perform(leftConverted, rightConverted)
	case "/":
		return divide{}.Perform(leftConverted, rightConverted)
	case "+":
		return plus{}.Perform(leftConverted, rightConverted)
	case "-":
		return minus{}.Perform(leftConverted, rightConverted)
	default:
